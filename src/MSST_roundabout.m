function [] = MSST_roundabout(times_number)
% only for roundabout
close all
%%  Configuration
% times_number = 2;
participant = 'P_熊宇轩';  
type = 'roundabout_look';
voicePath_store0  = strcat(participant,'\MSSTFeature\roundabout\look\MSSTFeature'); % save mat path
figureSavePath1 = strcat(participant,'\Figure\roundabout\'); % save gyry figure path
turn_start_offset = 2; % (default: +1)
no_time_record = 0;
...
BinsPath1 = strcat(participant,"\CollectedData\CollectedData_",type,"\CollectedData\BINS\TEST_BINS");
